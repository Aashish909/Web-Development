...
const Signin = () => {
   const [userData, setUserData] =useState({
    ...
    async function handleLogin(e) {
      e.preventDefault()
      try {
        // const data = await fetch("http://localhost:3000/api/v1/sign", {
        //   method: "POST",
...
<span className="material-symbols-outlined text-primary/40 text-5xl">edit_note</span>
      ...
      <form onSubmit={handleLogin} className="px-8 pb-10 space-y-5">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {/* <button
            ...
            type="button"
          >
            <span className="material-symbols-outlined text-xl mr-2 text-slate-900 dark:text-white">
              brand_family
            </span>
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
              GitHub
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 py-6 px-8 text-center border-t border-slate-200 dark:border-slate-800">
        <p className="text-sm text-slate-600 dark:text-slate-400">
          ...
          <Link to={"/signup"} className="font-bold text-primary hover:underline" href="#">
            {" "}
            Sign up for free
          </Link>
        </p>
